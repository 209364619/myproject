...
function aes_encrypt1() {
    ...
    console.log("本地解密后:" + decryptedStr);


    var pwd = "5obKQf+OIBGuQGIVi69LLQ==";
    //解密服务端返回的数据

    var decrypt = CryptoJS.AES.decrypt(pwd, key, {mode: CryptoJS.mode.ECB, padding: CryptoJS.pad.Pkcs7})
    var server_test = decrypt.toString(CryptoJS.enc.Utf8);
    console.log('模拟服务解密后：' + server_test);
}
...
function rsa_helper() {
    ...
    this.encrypt = function (text) {
        jsCrypter.setPublicKey(PUBLIC_KEY)
        return jsCrypter.encrypt(text)
    }
    this.decrypt = function (text) {
        jsCrypter.setPrivateKey(PRIVATE_KEY)
        return jsCrypter.decrypt(text)
    }
...
function aes_helper() {
     ...
     * 私有成员使用var进行定义
     *
     * 公有属性方法使用修饰符this进行修饰即可
     * @type {string}
     */
    var secret_key = CryptoJS.enc.Utf8.parse("0123456789123456");
    /**
     * 使用秘钥加密文本
     * @param text
     * @returns {string|*|PromiseLike<ArrayBuffer>}
     */
    this.encrypt_method = function (text) {
        var encryptData = CryptoJS.AES.encrypt(text, secret_key, {
            mode: CryptoJS.mode.ECB,
            padding: CryptoJS.pad.Pkcs7
        });
        return encryptData
     ...
     * @returns {string|*|PromiseLike<ArrayBuffer>}
     */
    this.decrypt_method = function (encryptText) {
        var decrypted = CryptoJS.AES.decrypt(encryptText, secret_key,
            {
                mode: CryptoJS.mode.ECB,
                padding: CryptoJS.pad.Pkcs7
            });
...
function aes_encrypt() {
    var aes = new aes_helper();
    password = $('#aes_input').val();
    encrypt_text = aes.encrypt_method(password);
    $('#aes_decrypt_input').val(encrypt_text);
    console.log($('#aes_encrypt_text'));
}

function aes_decrypt() {
    var aes = new aes_helper();
    encrypt_text = $('#aes_decrypt_input').val();
    decrypt_text = aes.decrypt_method(encrypt_text);
    $('#aes_decrypt_text').html("");
    $('#aes_decrypt_text').append(decrypt_text)
}

function rsa_encrypt() {
    password = $("#rsa_password").val();
    crypter = new rsa_helper();
    encrypt_result = crypter.encrypt(password);
    $("#ras_encrypt").html("")
    $("#ras_encrypt").val(encrypt_result)
}

function rsa_decrypt() {
    encrypt_text = $("#ras_encrypt").val();
    crypter = new rsa_helper();
    decrypt_text = crypter.decrypt(encrypt_text);
    $("#rsa_password").html("")
    $("#rsa_password").val(decrypt_text)
}
